...
good_linkedin_long <- good_linkedin |>
  select(company_name,
         ...
         `Staffing and Recruiting`) |>
  pivot_longer(
    cols = c(-company_name, -employee_count, -applies),
    names_to = "industry",
    values_to = "value"
  ) |>
  filter(value == 1) |>
...
good_linkedin_long |>
  ...
  labs(
    ...
    subtitle = "Applications Across Individual Job Postings",
    color = "Industry"
  ) +
...
good_linkedin_summary <- good_linkedin |>
  ...
  mutate(
    ...
    `Software Development` = sum(`Software Development`),
    `Financial Services` = sum(`Financial Services`),
    `Staffing and Recruiting` = sum(`Staffing and Recruiting`),
...
```{r Applications vs. Number of Postings}
apps_and_postings <- good_linkedin_long |>
  group_by(company_name) |>
  summarize(applications = sum(applies),
            postings = n())

apps_and_postings |>
  ggplot(aes(x = postings, y = applications)) +
  geom_point()
```
